...
public class EventFrontController {
    ...
    public void setCurrentUserEmail(String email) {
        ...
        if (userWelcomeLabel != null) userWelcomeLabel.setText("Bienvenue, " + email);
        loadRecommendations();
    }
    ...
    public void initialize() {
        try {
            ...
            if (currentUserEmail == null && SessionManager.getCurrentUser() != null) {
                currentUserEmail = SessionManager.getCurrentUser().getEmail();
                if (userWelcomeLabel != null) userWelcomeLabel.setText("Bienvenue, " + currentUserEmail);
            }
    ...
    private void loadEvents() {
        try {
            ...
            // Afficher les événements A_VENIR et EN_COURS (après normalisation du statut)
            // On exclut uniquement TERMINE et ANNULE
            List<Event> events = all.stream()
                    .filter(e -> {
                        String status = e.getStatus();
                        return status == null
                                || !Event.STATUS_TERMINE.equals(status) && !Event.STATUS_ANNULE.equals(status);
                    })
                    .toList();

            // Synchroniser current_capacity depuis la DB pour chaque événement
            // current_capacity = places RESTANTES = max_capacity - tickets_réservés
            for (Event e : events) {
                try {
                    int available = eventService.getAvailableSeatsFromDB(e.getId());
                    e.setCurrentCapacity(available);
                } catch (java.sql.SQLException ex) {
                    LOGGER.log(Level.WARNING, "Impossible de sync capacité event id=" + e.getId(), ex);
                }
            }
    ...
    private void loadRecommendations() {
        // Retirer la section reco existante si présente
        ...
        try {
            ...
            if (!recommendations.isEmpty()) {
                eventsContainer.getChildren().add(0, createRecommendationSection(recommendations));
            }
        } catch (SQLException e) {
    ...
    private VBox createRecommendationSection(List<Event> recommendations) {
        ...
        for (Event e : recommendations) cardsPane.getChildren().add(createCompactEventCard(e));
    ...
    private VBox createCompactEventCard(Event event) {
        ...
        Label dateLoc = new Label((event.getEventDate() != null ? event.getEventDate().format(dateFormatter) : "") + " | " + event.getLocation());
        ...
        card.getChildren().addAll(titleLbl, dateLoc, price);
        return card;
    }
    ...
    private void filterAndDisplay() {
        ...
        for (Event e : allEvents) {
            ...
            boolean matchCategory = "Toutes".equals(category) || category.equals(e.getCategory());
    ...
    private void displayEvents(ObservableList<Event> events) {
        ...
        if (recoSection != null) eventsContainer.getChildren().add(recoSection);
        ...
        Map<String, List<Event>> grouped = events.stream().collect(Collectors.groupingBy(Event::getCategory));
        for (Map.Entry<String, List<Event>> entry : grouped.entrySet()) {
            Label categoryTitle = new Label(entry.getKey());
            categoryTitle.setStyle("-fx-font-size: 24px; -fx-font-weight: bold; -fx-text-fill: #1a1a2e; -fx-padding: 20 0 10 20;");
            eventsContainer.getChildren().add(categoryTitle);

            TilePane cardsPane = new TilePane();
            cardsPane.setHgap(20); cardsPane.setVgap(20);
            cardsPane.setPrefColumns(3);
            cardsPane.setAlignment(Pos.TOP_LEFT);
            cardsPane.setPadding(new Insets(10, 0, 20, 0));
            for (Event ev : entry.getValue()) cardsPane.getChildren().add(createEventCard(ev));
    ...
    private VBox createEventCard(Event event) {
        VBox card = new VBox();
        card.setStyle("-fx-background-color: white; -fx-background-radius: 20; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.15), 15, 0, 0, 8); -fx-padding: 0 0 15 0; -fx-pref-width: 280px; -fx-min-width: 280px; -fx-max-width: 280px;");

        StackPane imageContainer = new StackPane();
        imageContainer.setPrefHeight(160);
        ImageView imageView = new ImageView();
        imageView.setFitWidth(280); imageView.setFitHeight(160); imageView.setPreserveRatio(false);
        if (event.getImageUrl() != null && !event.getImageUrl().isEmpty()) {
            imageView.setImage(ImageLoader.load(event.getImageUrl()));
        } else {
            imageView.setImage(ImageLoader.load(null));
        }
        ...
        Label dateBadge = new Label(event.getEventDate() != null ? event.getEventDate().format(dateFormatter) : "");
        ...
        imageContainer.getChildren().addAll(imageView, categoryBadge, dateBadge);
        ...
        listenBtn.setOnAction(e -> {
            ...
            int available = event.getCurrentCapacity();
            TextToSpeechUtil.speak(String.format("Événement : %s. Date : %s. Lieu : %s. Prix : %.2f DT. %d places disponibles.",
                    event.getTitle(), event.getEventDate() != null ? event.getEventDate().format(dateFormatter) : "",
        ...
        sponsorBtn.setStyle("-fx-background-color: #2c3e50; -fx-text-fill: white; -fx-font-weight: bold; -fx-background-radius: 25; -fx-padding: 8 20; -fx-cursor: hand;");
        sponsorBtn.setMaxWidth(Double.MAX_VALUE);
        sponsorBtn.setOnAction(e -> openPackCatalogue(event));

        HBox actionsBox = new HBox(10, reserveBtn, sponsorBtn);
        actionsBox.setAlignment(Pos.CENTER);
        HBox.setHgrow(reserveBtn, Priority.ALWAYS);
        HBox.setHgrow(sponsorBtn, Priority.ALWAYS);

        contentBox.getChildren().addAll(titleLbl, location, priceBox, availability, actionsBox);
        card.getChildren().addAll(imageContainer, contentBox);
        return card;
    }

    private void openPackCatalogue(Event event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/PackCatalogue.fxml"));
            Parent root = loader.load();
            PackCatalogueController controller = loader.getController();
            controller.setEvenementSelectionne(event);

            Stage stage = new Stage();
            stage.setScene(new Scene(root, 1000, 700));
            stage.setTitle("Packs de sponsoring - " + event.getTitle());
            stage.initModality(javafx.stage.Modality.APPLICATION_MODAL);
            stage.initOwner(eventsContainer.getScene().getWindow());
            stage.setResizable(true);
            stage.showAndWait();
        } catch (IOException e) {
            LOGGER.log(Level.SEVERE, "Erreur ouverture catalogue sponsoring", e);
            showAlert("Erreur", "Impossible d'ouvrir les packs de sponsoring.");
        }
    }
    ...
    @FXML private void handleMyReservations() {
        ...
        if (parentController != null) {
            parentController.showMesReservations(currentUserEmail);
        } else {
            // Fallback standalone (si ouvert hors user_home)
            try {
    ...
    @FXML private void handleMyDonations() {
        ...
        if (parentController != null) {
            parentController.showUserDonations(userId);
        } else {
            try {
                ...
                Stage stage = (Stage) eventsContainer.getScene().getWindow();
                stage.getScene().setRoot(root);
                stage.setTitle("Mes donations - Museum Digital");
            } catch (IOException e) {
                LOGGER.log(Level.SEVERE, "Erreur ouverture donations", e);
                showAlert("Erreur", "Impossible d'ouvrir vos donations.");
            }
        }
    }

    @FXML private void toggleChatbot() {
        try {
