...
public class SeatSelectionController {
    ...
    private void displayEventInfo() {
        ...
        // Le label sera mis à jour après generateSeatMap() qui connaît le vrai nombre de sièges libres
        if (availableSeatsLabel != null) availableSeatsLabel.setText("✅ Chargement...");
    ...
    /** Met à jour le label des places disponibles en comptant les sièges libres dans la grille. */
    private void updateAvailableSeatsLabel() {
        int freeSeats = 0;
        for (int r = 0; r < ROWS; r++)
            for (int c = 0; c < COLS; c++)
                if (!isReserved[r][c]) freeSeats++;
        if (availableSeatsLabel != null)
            availableSeatsLabel.setText("✅ " + freeSeats + " places disponibles");
    }
    ...
    private void generateSeatMap() {
        ...
        for (int r = 0; r < ROWS; r++) {
            for (int c = 0; c < COLS; c++) {
                ...
                if (reserved) {
                    // Block clicks without disabling (setDisable greys out the button and hides the blue color)
        ...
        // Mettre à jour le label avec le vrai nombre de sièges libres dans la grille
    ...
    private String getSeatStyle(boolean vip, boolean sel, boolean reserved) {
        if (reserved) return "-fx-background-color: #2196F3; -fx-text-fill: white; -fx-font-weight: bold; -fx-background-radius: 6; -fx-opacity: 1; -fx-cursor: default;";
