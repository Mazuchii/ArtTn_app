...
def extract_features(df):
    ...
    df['name_suspect'] = df['full_name'].apply(
        lambda x: 1 if pd.isna(x) or len(str(x)) <= 3 or str(x).lower() in ['test', 'user',  'temp', 'fake'] else 0
    )

    df['username_suspect'] = df['username'].apply(
        lambda x: 1 if pd.isna(x) or len(str(x)) < 3 or str(x).lower().startswith(('user', 'test', 'temp')) else 0
    )
