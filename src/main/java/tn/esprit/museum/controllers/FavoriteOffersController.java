...
public class FavoriteOffersController implements Initializable {
    ...
    public void setCandidatId(String candidatId) {
        if (candidatId != null && !candidatId.isBlank()) {
            ...
            // Charger les favoris après avoir défini l'ID
            chargerFavoris();
        }
    }
    ...
    private void chargerFavoris() {
        ...
        if (favoris.isEmpty()) {
            lblInfo.setText("Aucune offre favorite enregistrée pour cet identifiant.");
            return;
        }
        ...
        lblInfo.setText(favoris.size() + " offre(s) favorite(s) trouvée(s).");
    }
    ...
    private void supprimerFavori(OfferJob offre) {
        ...
        lblInfo.setText("L'offre a été retirée de vos favoris.");
        chargerFavoris();
    }
