...
import tn.esprit.museum.utils.ExcelExporter;
...
public class DemandeJobController implements Initializable {
    ...
    private String candidatId = null;
    ...
    public void setCandidatId(String candidatId) {
        ...
        if (tfCandidatId != null) {
            tfCandidatId.setText(candidatId);
            tfCandidatId.setEditable(false);
            tfCandidatId.setDisable(true);
        }
    }
    ...
    public void initDonneesOffre(int idOffre) {
        ...
        } else if (comboOffres != null) {
            // Chercher l'offre dans la liste pour la sélectionner par défaut
            for (String item : comboOffres.getItems()) {
                if (item.startsWith(idOffre + " - ")) {
                    comboOffres.setValue(item);
                    break;
                }
            }
        }
    ...
    void handleModifier() {
        ...
        try {
            URL resource = getClass().getResource("/fxml/AddDemandeForm.fxml");
            if (resource == null) {
    ...
    void handlePostuler() {
        if (!validerSaisie()) {
            showAlert("Champs incomplets", "Le CV est obligatoire.", Alert.AlertType.ERROR);
            return;
        }

        try {
            String finalCandidatId = (candidatId != null) ? candidatId : tfCandidatId.getText().trim();
            if (finalCandidatId == null || finalCandidatId.isEmpty()) {
                showAlert("Erreur", "ID du candidat introuvable.", Alert.AlertType.ERROR);
                return;
            }
            ...
            d.setCandidatId(finalCandidatId);
            ...
            // Utiliser tfCvUrl comme source car tfCandidatId a été supprimé du FXML
            handleCancel(new ActionEvent(tfCvUrl, null));
    ...
    @FXML void naviguerVersOffres(ActionEvent event) { changerScene(event, "/fxml/OfferWindow.fxml"); }
    @FXML void switchToFront(ActionEvent event) { changerScene(event, "/fxml/JobFront.fxml"); }
    ...
    private boolean validerSaisie() {
        if (tfCvUrl == null) return false;

        // Si on est un utilisateur, l'ID est déjà là, sinon on vérifie tfCandidatId
        boolean idOk = (candidatId != null) || (tfCandidatId != null && !tfCandidatId.getText().trim().isEmpty());

        return idOk && !tfCvUrl.getText().trim().isEmpty();
    }
    ...
    private void afficherScoreMatchingPourDemande(DemandeJob selected) {
        ...
        try {
            ...
            showAlert("Score matching impossible",
                    "Le calcul a echoue: " + e.getMessage(),
                    Alert.AlertType.ERROR);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            LOGGER.log(Level.SEVERE, "Score matching interrompu", e);
            showAlert("Score matching interrompu",
                    "Le calcul a ete interrompu. Veuillez reessayer.",
                    Alert.AlertType.ERROR);
        }
    }
    ...
    private void ajouterBoutonsAction() {
        colAction.setCellFactory(p -> new TableCell<>() {
            ...
            private final HBox actions = new HBox(8, btnCv, btnMeet, btnScore);
            {
                ...
                btnMeet.getStyleClass().add("btn-excel-meet");
