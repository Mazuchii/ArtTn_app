...
public class LoginController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if (languageCombo != null) {
            LanguageSelector.setup(languageCombo);
        }
        ...
        Platform.runLater(() -> {
            ...
            if (scene != null) {
                ...
                lm.applyOrientation(scene.getRoot());
            }
        });
    ...
    private void updateTexts() {
        Platform.runLater(() -> {
            if (loginTitleLabel  != null) loginTitleLabel.setText(lm.getString("login.title"));
            if (emailLabel       != null) emailLabel.setText(lm.getString("login.email"));
            if (passwordLabel    != null) passwordLabel.setText(lm.getString("login.password"));
            if (usernameField    != null) usernameField.setPromptText(lm.getString("login.email.prompt"));
            if (passwordField    != null) passwordField.setPromptText(lm.getString("login.password.prompt"));
            if (passwordVisible  != null) passwordVisible.setPromptText(lm.getString("login.password.prompt"));
            if (rememberMeCheckbox != null) rememberMeCheckbox.setText(lm.getString("login.remember"));
            if (forgotLink       != null) forgotLink.setText(lm.getString("login.forgot"));
            if (loginButton      != null) loginButton.setText(lm.getString("login.button"));
            if (googleLoginButton != null) googleLoginButton.setText("🅶 " + lm.getString("login.google"));
            if (noAccountLabel   != null) noAccountLabel.setText(lm.getString("login.no.account"));
            if (signupLink       != null) signupLink.setText(lm.getString("login.signup"));
            if (languageCombo    != null) {
                String code = lm.getCurrentCode();
                int idx = switch (code) { case "en" -> 1; case "ar" -> 2; default -> 0; };
                languageCombo.setOnAction(null);
                languageCombo.getSelectionModel().select(idx);
                LanguageSelector.setup(languageCombo);
            }
            if (usernameField != null && usernameField.getScene() != null) {
                lm.applyOrientation(usernameField.getScene().getRoot());
            }
        });
    }
    ...
    private void handleLogin() {
        ...
        try {
            ...
            if (user != null) {
                ...
                showError(lm.getString("login.error.invalid"));
            }
        } catch (Exception e) {
