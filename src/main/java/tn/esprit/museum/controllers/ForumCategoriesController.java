...
public class ForumCategoriesController {
    ...
    private void openCategoryPosts(int categoryId, String categoryName) {
        try {
            ...
            // ✅ Récupérer forumView à chaque fois via lookup (plus fiable)
            VBox forumView = (VBox) categoriesContainer.getScene().lookup("#forumView");

            if (forumView != null) {
                forumView.getChildren().clear();
                forumView.getChildren().add(postsView);
                System.out.println("✅ forumView trouvé et mis à jour");
            } else {
