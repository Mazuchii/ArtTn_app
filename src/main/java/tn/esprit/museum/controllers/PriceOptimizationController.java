...
public class PriceOptimizationController {
    ...
    public void initialize() {
        ...
        resultArea.setStyle("-fx-font-size: 13px; -fx-font-family: 'Segoe UI';");
        statusLabel.setText("Cliquez sur 'Analyser' pour obtenir des recommandations IA.");
    }
    ...
    private void handleAnalyze() {
        ...
        new Thread(() -> {
            try {
                ...
                String result = aiService.analyzeAndOptimizePrices(events, reservations);
                Platform.runLater(() -> {
                    resultArea.setText(result);
                    statusLabel.setText("✅ Analyse Gemini terminée — " + events.size() + " événements analysés.");
                    analyzeBtn.setDisable(false);
                    loadingBox.setVisible(false);
                });
            } catch (Exception e) {
                Platform.runLater(() -> {
                    ...
                    resultArea.setText("Erreur lors de l'analyse:\n" + e.getMessage() +
