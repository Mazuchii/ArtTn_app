...
public class SeatSelectionController {
    ...
    private static final Logger LOGGER =
            Logger.getLogger(SeatSelectionController.class.getName());
    ...
    // ===== Appelé depuis EventFrontController =====
    public void setEvent(Event event) {
        ...
        if (SessionManager.getCurrentUser() != null) {
            if (clientNameField  != null)
                clientNameField.setText(SessionManager.getCurrentUser().getFullName());
            if (clientEmailField != null)
                clientEmailField.setText(SessionManager.getCurrentUser().getEmail());
        }
    ...
    // ✅ Méthode appelée depuis EventFrontController pour passer l'email
    public void setUserEmail(String email) {
        if (clientEmailField != null && email != null)
    ...
    private void displayEventInfo() {
        ...
        String dateStr = currentEvent.getEventDate().format(dateFmt);
        String timeStr = currentEvent.getEventTime() != null
                ? currentEvent.getEventTime().format(timeFmt) : "N/A";
        eventDateTime.setText("🕐 " + dateStr + " à " + timeStr);
        eventPricePerTicket.setText(
                "💰 " + currentEvent.getPrice() + " DT standard | VIP +10 DT");

        int available = currentEvent.getMaxCapacity() - currentEvent.getCurrentCapacity();
        availableSeatsLabel.setText("✅ " + available + " places disponibles");

        if (currentEvent.getImageUrl() != null && !currentEvent.getImageUrl().isEmpty()) {
            try { eventImageView.setImage(new Image(currentEvent.getImageUrl())); }
            catch (Exception ignored) {}
        }
    }

    // ===== Génération plan de salle =====
    private void generateSeatMap() {
        ...
        for (int r = 0; r < ROWS; r++) {
            for (int c = 0; c < COLS; c++) {
                ...
                Button btn = new Button(getSeatLabel(r, c));
                btn.setPrefSize(42, 38);
                btn.setStyle(getSeatStyle(vip, false));
    ...
    private String getSeatStyle(boolean vip, boolean sel) {
        if (sel)  return "-fx-background-color: #2196F3; -fx-text-fill: white; "
                + "-fx-font-weight: bold; -fx-background-radius: 6;";
        if (vip)  return "-fx-background-color: #4CAF50; -fx-text-fill: white; "
                + "-fx-font-weight: bold; -fx-background-radius: 6;";
        return "-fx-background-color: #D4AF37; -fx-text-fill: #1a1a2e; "
                + "-fx-font-weight: bold; -fx-background-radius: 6;";
    }

    private double getSeatPrice(boolean vip) {
        return currentEvent.getPrice().doubleValue() + (vip ? 10 : 0);
    }
    ...
    private void toggleSeat(int r, int c) {
        ...
        seatButtons[r][c].setStyle(getSeatStyle(isVip[r][c], selected[r][c]));
        updateSummary();
    }

    private void updateSummary() {
        selectedSeatsLabel.setText(selectedSeats.isEmpty()
        ...
        totalPriceLabel.setText(String.format("Total : %.2f DT", currentTotal));
    }
    ...
    private void handleConfirm() {
        ...
        if (selectedSeats.isEmpty()) {
            showAlert("Erreur", "Veuillez sélectionner au moins un siège.");
            return;
        }
        ...
        // Si session active, utilise les données de session
        ...
        int tickets = selectedSeats.size();

        System.out.println("name=" + name + " email=" + email
                + " tickets=" + tickets + " total=" + currentTotal);
        ...
        try {
            ...
            r.setSpecialRequests("Sièges: " + String.join(", ", selectedSeats)
                    ...
                    ? "" : " | Promo: " + promoCodeField.getText()));
            r.setSelectedSeats(String.join(",", selectedSeats));

            reservationService.insert(r);
            System.out.println("✅ Réservation insérée ID=" + r.getId());
            ...
            showAlert("✅ Réservation confirmée !",
                    "Client : " + name
                            ...
                            + (!promoCodeField.getText().isEmpty()
                            ? "\nCode promo : " + promoCodeField.getText() : ""));
            closeWindow();
