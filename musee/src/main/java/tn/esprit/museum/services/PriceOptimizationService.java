...
public class PriceOptimizationService {
    ...
    // ⚙️ Configuration API Gemini (remplacez par votre clé et modèle)
    ...
    // ==================== ANALYSE GLOBALE (tous les événements) ====================
    public String analyzeAndOptimizePrices(List<Event> events, List<Reservation> reservations) throws IOException {
        ...
        prompt.append("Termine par un résumé global avec:\n");
        prompt.append("- Revenu actuel estimé\n");
        prompt.append("- Revenu potentiel après optimisation\n");
        prompt.append("- 2-3 conseils stratégiques pour le musée\n");
    ...
    public PriceSuggestion suggestPriceForEvent(Event event, List<Reservation> allReservations) {
        try {
            System.out.println("📡 Appel API Gemini pour : " + event.getTitle());
            ...
            System.err.println("⚠️ Erreur API, fallback local : " + e.getMessage());
    ...
    private String buildSingleEventPrompt(Event event, List<Reservation> allReservations) {
        ...
        int fillRate = event.getMaxCapacity() > 0
                ? (event.getCurrentCapacity() * 100) / event.getMaxCapacity()
                : 0;
        ...
        prompt.append("- Réservations confirmées pour cet événement : ").append(eventReservations).append("\n");
        prompt.append("Propose un prix optimisé (une valeur numérique simple, par exemple 27.50).\n");
        prompt.append("Si le remplissage est très bas (<20%), suggère une baisse de 10-20%.\n");
        prompt.append("Si le remplissage est moyen (20%-70%), garde le prix ou ajuste légèrement.\n");
        prompt.append("Si le remplissage est élevé (>70%), suggère une augmentation modérée.\n");
    ...
    private String callGemini(String prompt, int maxTokens) throws IOException {
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (!response.isSuccessful()) {
                throw new IOException("Erreur API (" + response.code() + "): " + responseBody);
            }
    ...
    private PriceSuggestion parseSuggestion(String geminiResponse, double currentPrice) {
        try {
            ...
            // Sinon, chercher un nombre décimal dans la réponse
    ...
    // ==================== FALLBACK LOCAL INTELLIGENT (sans API) ====================
    private PriceSuggestion fallbackSuggestion(Event event) {
        ...
        // Facteur d'ajustement selon la catégorie
        double categoryFactor = switch (event.getCategory()) {
            case "SPECTACLE" -> 1.05;
        ...
        if (fillRate < 20) {
            ...
            reason = "Faible remplissage, baisse recommandée (catégorie : " + event.getCategory() + ")";
        } else if (fillRate > 70) {
            suggested = currentPrice * 1.15 * categoryFactor;
            reason = "Forte demande, augmentation possible (catégorie : " + event.getCategory() + ")";
        } else {
